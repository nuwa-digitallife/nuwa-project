...
def create_llm_by_provider(provider: str, model: str, backend_url: str, temperature: float, max_tokens: int, timeout: int, api_key: str = None):
    ...
    elif provider.lower() == "claude-cli":
        from tradingagents.llm_adapters.claude_cli_llm import ClaudeCLILLM
        logger.info(f"🔧 [ClaudeCLI] 使用 claude -p 订阅模式，model={model}")
...
class TradingAgentsGraph:
    ...
    def __init__(
        ...
        elif self.config["llm_provider"].lower() == "claude-cli":
            # Claude CLI 订阅模式：无需 API Key，通过 claude -p 命令行调用
            from tradingagents.llm_adapters.claude_cli_llm import ClaudeCLILLM
            logger.info(f"🔧 [ClaudeCLI] 使用 claude -p 订阅模式")
            logger.info(f"   deep_model={self.config['deep_think_llm']}, quick_model={self.config['quick_think_llm']}")
            self.deep_thinking_llm = ClaudeCLILLM(
                model=self.config["deep_think_llm"],
                timeout=deep_timeout
            )
            self.quick_thinking_llm = ClaudeCLILLM(
                model=self.config["quick_think_llm"],
                timeout=quick_timeout
            )
            logger.info("✅ [ClaudeCLI] LLM 实例创建完成")
        else:
