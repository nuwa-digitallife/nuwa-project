...
"""
                ...
                # 有工具调用，但工具未实际执行（LangChain tool-calling 需手动调用）
                # 提取工具参数，手动执行，再让 LLM 生成报告
                try:
                    tool_call = result.tool_calls[0]
                    tool_args = tool_call.get('args', {})
                    logger.info(f"[新闻分析师] 🔧 手动执行待处理工具: {tool_call.get('name')}, 参数: {tool_args}")

                    executed_news = unified_news_tool(
                        stock_code=tool_args.get('stock_code', ticker),
                        max_news=tool_args.get('max_news', 10),
                        model_info=model_info
                    )
                    logger.info(f"[新闻分析师] 📋 工具执行结果长度: {len(executed_news) if executed_news else 0} 字符")

                    if executed_news and len(executed_news.strip()) > 100:
                        analysis_prompt = f"""您是一位专业的财经新闻分析师。请基于以下获取的新闻数据，对股票 {ticker}（{company_name}）进行详细的新闻分析：
...
请撰写详细的中文分析报告。"""
                        rerun_result = llm.invoke([{"role": "user", "content": analysis_prompt}])
                        report = rerun_result.content if hasattr(rerun_result, 'content') else str(result.content)
                        logger.info(f"[新闻分析师] ✅ 工具执行后重新生成报告，长度: {len(report) if isinstance(report, str) else 'list'} 字符")
                    else:
                        logger.warning(f"[新闻分析师] ⚠️ 工具返回内容过短，使用LLM原始内容")
                        report = result.content if hasattr(result, 'content') else ""
                except Exception as e:
                    logger.error(f"[新闻分析师] ❌ 手动工具执行失败: {e}")
                    report = result.content if hasattr(result, 'content') else ""
