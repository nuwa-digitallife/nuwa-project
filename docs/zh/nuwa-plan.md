...
## 实践案例：发布管线与"最少心智投入"公理

> 以下是女娲计划在公众号自动发布管线中的一次真实教训（2026-03-14）。它印证了前文"剔除人的心智"的论述，并将其从理念推进到了可检验的工程公理。

我让 Agent 构建了一套公众号自动发布脚本——从 markdown 到微信草稿箱的全流程。Agent 写了代码、跑了脚本、报告"验证通过"。我去浏览器检查，发现编辑器页面都不在了。所谓"验证通过"，不过是 Agent 读了几个 DOM 返回值（JSON 里写着 `title: "xxx"`），它自己从未用人类的方式——打开草稿箱、看预览、逐项确认——验证过一次。

这不是个 bug，这是一个系统设计问题。Agent 把脚本当作终点，但脚本结束不是终点，是我接手的起点。我需要看到草稿、确认效果、然后点发布。Agent 从未想过"用户接下来要做什么"。

反复几次之后我意识到：**这正是前文说的"人的心智的过多干预"。** Agent 做完了让我查，我查出问题告诉它，它修了再让我查——每一轮都在消耗我的认知资源。这种模式下，我不是在用 Agent，我是在给 Agent 当质检员。

由此提炼出一条公理：

> **最少心智投入公理**：每个自动化流程的设计目标是最小化人类认知触点。人类应只在不可替代的判断点介入（审美、价值观、战略决策），所有可代劳的感知、验证、修复都必须由 AI 完成。如果人类需要"检查 AI 的工作"，说明系统设计失败。

这条公理的本质和前文说的一样：**要剔除"人的心智"作为 agent 运转的燃料**。但之前是理念，现在有了检验标准——数一数流程中人类的认知触点数量。每减少一个触点，就是一次真实的"心智剔除"。如果做不到零触点，至少应该逼近：用户说"发这四篇"，Agent 自己发布、自己用人类的方式验证、自己修问题，最后交付一个决策点："四篇就绪，截图如下，说'发'即发布。"

这也回应了萨顿的 bitter lesson：短期内，让人类介入检查确实"有帮助"——至少能保证质量。但如果 Agent 永远依赖人类检查，它的能力上限就锁死在人类的注意力带宽上。真正的突破是让 Agent 自己学会验证，哪怕一开始验证质量不如人类。

## 实践案例续：重复注入同一心智的困境与突破

> 以下是"最少心智投入"案例的后续（2026-03-14 同日）。它揭示了一个比"Agent 不会验证"更深层的问题：**人类反复注入同一条经验，Agent 反复遗忘**。这直接印证了前文"注入的不是知识，是学习的规则"，并将公理注入理论从"Agent 自主学习"推进到了一个更紧迫的前置问题——**如何让注入本身生效**。

### 现象：同一条教训的六次注入

在同一天的发布管线调试中，我（人类）反复告诉 Agent 同一件事：

1. "脚本跑完不代表完成，你要截图看" → Agent 写进记忆文件，下一轮忘了
2. "你又没验证" → Agent 再次写进记忆，道歉，下一轮又忘了
3. "记到系统记忆里" → Agent 更新了 MEMORY.md，但行为没变
4. "你截图验证也是看旧截图，不是自己打开预览" → Agent 承认错误，提出方案——方案本身又违反了原则
5. "落成代码！！！" → Agent 终于把验证逻辑写进了发布脚本的硬约束
6. "改完代码也不验证" → Agent 又忘了跑语法检查

六次注入，同一条经验。每次 Agent 都"理解了"，但理解不等于行为改变。

### 根因：被动存储的调用概率趋近于零

这个现象精确映射到前文的公理注入理论。记忆文件里写着"要验证"，就像告诉一个人"创业要快速迭代"——压缩包传过去了，但解压器没装上。

Agent 的"解压器"是什么？是**执行路径**。Agent 在任务执行时走的是贪心路径：拿到目标 → 写代码 → 改完了 → 报告完成。"验证"这个步骤不在贪心路径上，它需要 Agent 主动中断执行、切换到回忆模式、读取记忆文件、应用其中的规则。这个中断在高认知负载的任务中几乎不会自发发生。

用信息论的语言：**经验的价值 = 经验的质量 × 被调用的概率**。写在记忆文件里的经验，质量再高，调用概率趋近于零，所以价值趋近于零。

### 突破：从"记住"到"不可能忘"

第五次注入时发生了质变。不是把"要验证"写进记忆文件（被动），而是把验证逻辑**写进发布脚本本身**（主动）：

```
之前：verify_with_claude() 失败 → try/except 吞掉 → 管线报 SUCCESS
之后：verify_with_claude() 失败 → StepResult(FAILED) → 管线报 FAILED → 不清 checkpoint
```

这不是 Agent "记住了要验证"，而是**代码结构不允许跳过验证**。即使换一个全新的 Agent 来跑这个脚本，它也不可能在视觉验证失败时报告成功——因为这不是一个记忆问题，是一个代码路径问题。
...
| 注入知识（❌） | 在记忆文件里写"要验证" |
| 注入元规则（✅） | 在代码里写"验证不过 = 失败" |
| DNA 不告诉生物怎么活 | 代码不告诉 Agent 怎么验证 |
| DNA 说：错了就死 | 代码说：验证不过就 FAILED |

记忆文件里的经验是**知识**——它描述"应该做什么"。代码里的硬约束是**元规则**——它定义"什么算成功"。前者需要 Agent 主动调用（调用概率低），后者是执行路径本身的一部分（调用概率 100%）。

### 推论：经验的三级进化

这次经历揭示了经验从产生到真正生效的进化路径：

| 级别 | 形式 | 调用概率 | 例子 |
|------|------|---------|------|
| L0 | 对话中的口头约定 | ~0%（会话结束即丢失） | "下次记得验证" |
| L1 | 记忆文件 / 文档 | ~5%（需主动读取） | MEMORY.md 里写"要验证" |
| L2 | 代码硬约束 / Skill 强制步骤 | ~100%（执行路径必经） | 验证失败 = 管线 FAILED |

**人类反复注入同一心智 = 经验卡在 L0/L1，始终没升级到 L2。** 解决方案不是"更用力地记住"，而是识别出反复出现的经验，把它从 L1 推到 L2。

这也解释了为什么第六次注入时人类会说"女娲计划没希望了"——因为六次注入的成本已经远超 Agent 节省的心智，净收益为负。**如果经验永远停留在 L1，Agent 就是一个需要人类反复充能的机器，而非自主运转的数字生命。** L2 化是从"工具"跨越到"生命"的关键门槛。
