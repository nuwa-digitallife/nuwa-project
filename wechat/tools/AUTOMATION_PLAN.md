...
**当前聚焦：发布管线修复（P0）+ Agent PM 架构设计 + 审稿**

**当前状态**：写作引擎完成，发布管线实测中（粘贴/图片/mdnice/封面 ✅，标题/简介/原创/赞赏/投票 ❌ 待修）。根因：Playwright .click() 在微信编辑器中系统性失效，需全面改 JS evaluate。
...
| 4.4 | 标题/简介/封面设置 | 🟡 部分工作 | `wechat_automator.py` _cdp_type() | 封面 ✅，标题/简介已改 JS focus 待测试 |
| 4.5 | 原创声明+赞赏 | 🔴 有 bug | `wechat_automator.py` enable_original/reward | 原创 switch 找不到、赞赏确认按钮找不到。作者字段已修复（不用填） |
| 4.6 | 投票创建 | 🟡 代码完成 | `wechat_automator.py` add_poll() | _cdp_type 已改 JS focus，待测试 |
| 4.7 | 端到端发布脚本 | 🔴 实测中 | `publish/one_click_publish.py` | 粘贴✅ 图片✅ mdnice✅ 封面✅。标题/简介/原创/赞赏/投票 ❌，详见下方 |

**WS-4 发布管线当前状态（2026-03-14 实测）**：

已通过的步骤：
- ✅ Chrome CDP 自动启动（无 Chrome 时自动启动，已有 Chrome 时直接连接，不杀进程）
- ✅ 图片上传到微信 CDN（9张全成功）
- ✅ mdnice 排版（橙心主题，复制到剪贴板）
- ✅ 正文粘贴（JS focus + Cmd+V，DOM 验证 3/3 探针 + 5/5 配图）
- ✅ 封面图设置（JS click 修复后成功）

待修复的步骤：
- ❌ 标题/简介设置 — `_cdp_type()` 已改 JS focus 但未测试通过
- ❌ 原创声明 — `enable_original_only.js` selector 在某些状态下找不到 switch
- ❌ 赞赏 — 对话框确认按钮找不到
- ❌ 投票填写 — `_cdp_type()` 已改但未测试

系统性根因：**Playwright .click() 在微信编辑器中全面失效**（"outside viewport" / "<html> intercepts"）。所有交互必须用 JS evaluate。已修复 paste/cdp_type/cover，其余待全面排查。

需要新增的能力：
- one_click_publish.py 需要 **resume 机制**（跳过已完成步骤，只跑失败步骤）
- 调试时应能单独跑某个步骤（如 `python wechat_automator.py set-title "xxx"`）
...
| 10.4 | 「技术祛魅」系列试写 | 🟡 引擎已生成 | — | 世界模型 + Anthropic-Skill转向 已由引擎生成全套交付物，待人工审稿 |
| 10.5 | 「涟漪」系列试写 | 🟡 引擎已生成 | — | OpenClaw算力涟漪 + 中国AI-Token超美 已由引擎生成全套交付物，待人工审稿 |
| 10.6 | 「先驱者」系列试写 | 🟡 引擎已生成 | — | 三局棋（哈萨比斯）已由引擎生成全套交付物，待人工审稿 |
...
**当前焦点：Agent PM Phase 1 实现（女娲第一个数字生命）**
1. **WS-11.0 前置准备** — ✅ 计划已存 `wechat/agent/PLAN_agent_pm.md`，待用户创建 Telegram Bot
2. **WS-11.1-11.4 Phase 1 骨架** — ⬜ daemon + brain + gateway + state + memory + 1个 skill
3. **WS-4.7/8.2 发布实测** — 🟡 Mac Mini 发布中（并行进行）
4. **WS-11.5-11.6 Phase 2 完整技能** — ⬜ 封装全部工具 + 文章全生命周期
5. **WS-11.7-11.9 Phase 3 主动进化** — ⬜ 主动思考 + 路径压缩 + 商业化分析
